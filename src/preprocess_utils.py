...
def add_mol(mol_dict, mol):
    ...
    atom_fea8 = np.array([_chirality(a) for a in mol.GetAtoms()])
    atom_fea9 = np.array(
        [[a.IsInRingSize(s) for s in ringsize_list] for a in mol.GetAtoms()],
        
    )
    atom_fea10 = np.array(
        [[a.GetIsAromatic(), a.IsInRing()] for a in mol.GetAtoms()]
    )
    ...
    if n_edge > 0:
        bond_fea1 = np.eye(len(bond_list))[
            [bond_list.index(str(b.GetBondType())) for b in mol.GetBonds()]
        ]
        bond_fea2 = np.array([_stereochemistry(b) for b in mol.GetBonds()])
        bond_fea3 = np.array(
