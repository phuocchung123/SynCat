...
class GraphDataset:
    ...
    def __getitem__(self,idx):
        ...
        for j in range(self.rmol_max_cnt):
            ...
            r_edge_attr=torch.from_numpy(self.rmol_edge_attr[j][self.rmol_e_csum[j][idx]:self.rmol_e_csum[j][idx+1]]).float()

            r_node_attr=torch.from_numpy(self.rmol_node_attr[j][self.rmol_n_csum[j][idx]:self.rmol_n_csum[j][idx+1]]).float()
        ...
        for j in range(self.pmol_max_cnt):
            ...
            p_edge_attr=torch.from_numpy(self.pmol_edge_attr[j][self.pmol_e_csum[j][idx]:self.pmol_e_csum[j][idx+1]]).float()

            p_node_attr=torch.from_numpy(self.pmol_node_attr[j][self.pmol_n_csum[j][idx]:self.pmol_n_csum[j][idx+1]]).float()
