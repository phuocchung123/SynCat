...
class SmilesFeaturizer:
    ...
    @staticmethod
    def get_ecfp(mol: Chem.Mol, radius: int, nBits: int = 2048, useFeatures: bool = False):
        """
        Generates Extended-Connectivity Fingerprints (ECFP) or Feature-Class Fingerprints (FCFP) from an RDKit Mol object.
        ...
        """
        return AllChem.GetMorganFingerprintAsBitVect(mol, radius, nBits=nBits, useFeatures=useFeatures)

    @staticmethod
    def get_rdk_fp(mol: Chem.Mol, maxPath: int, fpSize: int = 2048, nBitsPerHash: int = 2):
        """
        ...
        """
        return Chem.RDKFingerprint(mol, maxPath=maxPath, fpSize=fpSize, nBitsPerHash=nBitsPerHash)
    ...
    @classmethod
    def featurize_smiles(cls, smiles: str, fingerprint_type: str, convert_to_array: bool = True, **kwargs) -> np.ndarray:
        """
